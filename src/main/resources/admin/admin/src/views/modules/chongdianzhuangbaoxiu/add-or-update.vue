...
export default {
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `chongdianzhuangbaoxiu/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.guzhangmiaoshu = this.ruleForm.guzhangmiaoshu.replace(reg,'../../../springboot3l3fa/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `chongdianzhuangbaoxiu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.chongdianzhuangbaoxiuCrossAddOrUpdateFlag = false;
						 this.parent.search();
