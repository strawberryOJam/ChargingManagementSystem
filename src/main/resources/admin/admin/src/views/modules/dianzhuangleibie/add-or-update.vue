...
export default {
  data() {
	...
	dianzhuangleibie : false,
      ...
      ruleForm: {
       dianzhuangleibie: '落地式,挂壁式',
      },
      rules: {
          dianzhuangleibie: [
                { required: true, message: '电桩类别不能为空', trigger: 'blur' },
          ],
      }
    };
  },
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `dianzhuangleibie/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.dianzhuangleibieCrossAddOrUpdateFlag = false;
						 this.parent.search();
